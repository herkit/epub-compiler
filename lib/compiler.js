...
module.exports.compile = function(project) {
  ...
  console.log(flattenContent(project, ''));
  ...
  .pipe(fs.createWriteStream(project.output.filename))
  .on('finish', function () {
      console.log(project.output.filename + " written.");
  });
}

function flattenContent(item, basePath) {
  switch (item.type) {
    case "file":
      return [ {
    ...
    case "folder":
      var result = item
        ...
        .map(function(subitem) { 
          return flattenContent(subitem, path.posix.join(basePath, item.name || ''));
        })
